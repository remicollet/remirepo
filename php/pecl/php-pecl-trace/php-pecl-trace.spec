...
extver=$(sed -n '/#define TRACE_VERSION /{s/.* "//;s/".*$//;p}' ../common/trace_version.h)
...
%build
%{?dtsenable}
cd %{pecl_name}-%{version}%{?prever}/extension
...
do install -Dpm 644 %{pecl_name}-%{version}%{?prever}/$i %{buildroot}%{pecl_docdir}/%{pecl_name}/$i
done
